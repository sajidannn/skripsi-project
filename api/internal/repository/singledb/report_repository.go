...
// ReportRepo implements repository.ReportRepository for single-DB mode.
type ReportRepo struct {
	db *pgxpool.Pool
}

// NewReportRepo creates a new ReportRepo backed by the shared pool.
...
func (r *ReportRepo) GetBranchBalance(ctx context.Context, tenantID, branchID int, filter dto.ReportFilter) (*model.BranchBalance, error) {
	...
	query := fmt.Sprintf(`
		...
		FROM branches b
		LEFT JOIN branch_cashflow bc ON bc.branch_id = b.id AND bc.tenant_id = b.tenant_id %s
	...
	if err != nil {
		...
		return nil, fmt.Errorf("singledb.ReportRepo.GetBranchBalance: %w", err)
	}
...
func (r *ReportRepo) GetTenantBalance(ctx context.Context, tenantID int, filter dto.ReportFilter) (*model.TenantBalance, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("singledb.ReportRepo.GetTenantBalance: %w", err)
	}
...
func (r *ReportRepo) InjectCapital(ctx context.Context, tenantID int, req dto.CapitalRequest) error {
	query := `
		INSERT INTO tenant_cashflow (tenant_id, flow_type, direction, amount)
		VALUES ($1, 'ADJUSTMENT', $2, $3)
	`
	_, err := r.db.Exec(ctx, query, tenantID, req.Direction, req.Amount)
	if err != nil {
		return fmt.Errorf("singledb.ReportRepo.InjectCapital: %w", err)
	}
	return nil
}
...
func (r *ReportRepo) GetSummary(ctx context.Context, tenantID int, filter dto.SummaryFilter) (*model.ReportSummary, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("singledb.ReportRepo.GetSummary: %w", err)
	}
	return &s, nil
}
...
func (r *ReportRepo) GetItemPerformance(ctx context.Context, tenantID int, filter dto.ItemsFilter, ascending bool) ([]model.ItemPerformance, error) {
	...
	query := fmt.Sprintf(`
		...
		WHERE t.tenant_id = $1 AND t.trans_type = 'SALE' %s
	...
	if err != nil {
		return nil, fmt.Errorf("singledb.ReportRepo.GetItemPerformance: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ItemID, &item.ItemName, &item.SKU,
			...
			return nil, fmt.Errorf("singledb.ReportRepo.GetItemPerformance scan: %w", err)
		}
...
func (r *ReportRepo) GetSalesReport(ctx context.Context, tenantID int, filter dto.SalesReportFilter) ([]model.SalesReportEntry, error) {
	args := []any{tenantID, *filter.DateFrom, *filter.DateTo}
	where := ""
	...
	rows, err := r.db.Query(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("singledb.ReportRepo.GetSalesReport: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&entry.Date, &entry.TotalTransactions,
			...
			return nil, fmt.Errorf("singledb.ReportRepo.GetSalesReport scan: %w", err)
		}
