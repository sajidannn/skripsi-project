...
// If additive=false, it TRUNCATES all tables first (cascade), then bulk-inserts all data.
// If additive=true, it skips TRUNCATE and only inserts the new tenants generated by gen.
func SeedSingle(ctx context.Context, pool *pgxpool.Pool, gen *Generator, additive bool) error {
	if !additive {
		...
		log.Printf("[single] Additive mode: skipping TRUNCATE, inserting %d new tenant(s) starting from ID %d",
